...
function playSound() {
       ...
       /*  if (document.visibilityState === 'visible') 
            ...
            // 
                ...
                //const notifySound = new Audio('/home.app/tg.mp3');
                const notifySound = document.getElementById('clickSound');
                notifySound.play().catch(function(error) {
                    console.log("🤫 Браузер заблокировал звук, так как вы еще не кликали по странице.");
                });
...
async function modakInfoUser(){
    ...
    if(data.mes !== "no") {
        ...
        let statusElement = document.getElementById('info-user-status');
...
async function ObnowInfoOfUser() {
   ...
   let em = document.getElementById('newEmail').value.trim();
    let passw = document.getElementById("newPasUs").value.trim();
...
function scrollToBottom() {
    ...
    // Используем setTimeout(..., 0), чтобы дать браузеру один "тик" на отрисовку
    setTimeout(() => {
        messagesContainer.scrollTo({
            top: messagesContainer.scrollHeight,
            behavior: 'smooth' // добавит плавности
        });
    }, 50); 
}
...
async function SendMes(){
   ...
   // t.classList.add('me');
    ...
    if(inp.trim() == "" || chat ==""){
     ...
     ///////////////////////////////////////
   let txt = inp.trim();
     if(txt.split(':')[0] == "http" ||txt.split(':')[0] == "https" ){
        let a = document.createElement('a');// ТЕКСТ
        a.textContent = txt;
        a.classList.add('me');
...
function MesOt(mes,username,time,id,type,fileU,fileSize,fileName){
    
    let dd = document.createElement('div');// USERNAME
    ...
    let div = document.querySelector('.messages');
    let t = document.createElement('div'); // TIMe
    t.textContent = time;
    t.classList.add('message-time');
    ...
    if(mes == ""){
      ...
      switch(type){
        case 'audio':
            ...
            audio.className = 'chat-audio other-audio';
            audio.controls = true;
             audio.src = fileU;
        ...
        case 'image':
            ...
            img.src = fileU;
            img.className = 'chat-img img-oth';
        ...
        let video = document.createElement('video');
           ...
           /* video.onclick = function() {
            ...
            video.className = 'chat-img img-oth';
            video.controls = true;
            video.dataset.id = id;
            div.appendChild(video);
        // t.classList.add('img-my-time');
            div.appendChild(t);
            break;
            ...
            let allF = document.createElement('div');
            allF.className = 'chat-file other-file';
            allF.dataset.id = id;
            ...
            fileinfo.className = 'file-info';
                ...
                filesize.textContent = fileSize + " байт";
            ...
            allF.appendChild(span);
            allF.appendChild(fileinfo);
            allF.appendChild(a);

            div.appendChild(allF);
        ...
        let tip = mes.split(':')[0];
            if(tip == "http" || tip == "https"){
                ...
                let p = document.createElement('p');// ТЕКСТ
                p.textContent = mes;
                p.classList.add('oth');
                p.dataset.id = id;
...
if (data.length === lastCount) {
    ...
    for(let i = 0; i < data.length; i++) {
        ...
        // Если это картинка — пропускаем проверку текста
     if (data[i].type === "image" ||data[i].type === "video" ||data[i].type === "audio" ||data[i].type === "textfile") {
        continue; // не проверяем картинки
    }
    ...
    for(let i = 0; i < data.length; i++){
        ...
        if(usIdd == userId){
           ...
           /* video.onclick = function() {
            ...
            video.className = 'chat-img img-my';
            video.controls = true;
            video.dataset.id = id;
            div.appendChild(video);
            t.classList.add('img-my-time');
           ...
           let audio = document.createElement('audio');
           audio.className = 'chat-audio my-audio';
           audio.controls = true;
            audio.src = fileU;
            /*audio.onclick = function() {
                openPhoto(this.src);
           ...
           case 'textfile':
            let allF = document.createElement('div');
            allF.className = 'chat-file my-file';
            allF.dataset.id = id;
            let span = document.createElement('span');
            span.textContent = '📄';
            span.className = 'file-icon';
            ...
            fileinfo.className = 'file-info';
                ...
                let filename = document.createElement('div');
                filename.className = 'file-name';
                filename.textContent = data[i].fileName;

                let filesize = document.createElement('div');
                filesize.className = 'file-size';
                filesize.textContent = data[i].fileSize + " байт";

            let a = document.createElement('a');
            a.className = 'file-download';
            a.href = fileU;
            a.download = data[i].fileName;
            a.textContent = "↓";
             a.title="скачать"
            fileinfo.appendChild(filename);
            fileinfo.appendChild(filesize);
           ...
           case 'text':
            ...
            MesOt(text , username,tim,id,type,fileU,data[i].fileSize,data[i].fileName);
         }
...
all.forEach(div => {
    ...
    let oldCount = lastUnreadCounts[chatName] || 0;
    if (badge) {
        if (count > 0) {
          if(lazyUser && count > oldCount){
            lastUnreadCounts[chatName] = count;
            playSound();
            console.log("e");
        }
...
async function pingU() {
    const response = await fetch('/api/chotoTam/ping', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function getUserInfo() {
    ...
    if(data.message === "no"){
        ...
        idUser = data.id;
        document.getElementById('newUsNa').innerHTML = data.username;
...
// Резервный вариант для http://192.168.0.116
...
function resetTimer() {
    lazyUser = false;
    console.log("e");
    clearTimeout(timer); // Отменяем старый будильник
    timer = setTimeout(ABCD, 30000); // Ставим новый на 1 минуту (60000 мс)
}
...
async function SendFile(fie) {
    ...
    formData.append('username', user); // Твой ID из localStorage
    formData.append('file', fie); // Твое имя
    formData.append('chat', chat); // Текущий чат
...
document.querySelector('.messages').addEventListener('contextmenu', function(e) {
    ...
    const msg = e.target.closest('p, a.me, a.oth, img, video, audio, .chat-file');
    if (msg /*&& (msg.classList.contains('me') || msg.classList.contains('oth'))*/) {
       ...
       // 2. "Копировать" и "Редактировать" — только если это ТЕКСТ (p или a)
    const isText = msg.tagName === 'P' || msg.tagName === 'A';
...
// 3. Слушаем события активности
window.onload = resetTimer; // Запуск при загрузке страницы
window.onmousemove = resetTimer; // Движение мыши
window.onmousedown = resetTimer; // Клики
window.ontouchstart = resetTimer; // Тапы на телефонах
window.onkeydown = resetTimer; // Нажатие клавиш
...
// КОГДА ПОЛЬЗОВАТЕЛЬ ВСТАВЛЯЕТ ЧТО-ТО (CTRL+V) В ПОЛЕ ВВОДА
document.getElementById("Send").addEventListener("paste", function(e) {
    
    // items - это ВСЁ, что лежит в буфере обмена (картинки, текст и т.д.)
    ...
    for (let item of items) {
        ...
        if (item.type.includes("image")) {
            ...
            // КОГДА файл прочитается - делаем это:
...
// КОГДА ПОЛЬЗОВАТЕЛЬ ВЫБИРАЕТ ФАЙЛ ЧЕРЕЗ СКРЕПКУ
document.getElementById('addFile').addEventListener('change', function(e) {
    
    // e.target.files[0] - это выбранный файл
